using PartiallyObservedMarkovProcesses
import PartiallyObservedMarkovProcesses as POMP
using Random: seed!
using Test

@info h1("testing constructors and workhorses")

@testset verbose=true "constructor and workhorses" begin

    seed!(263260083)

    include("test1.jl")

    @test_throws "must be no later than" pomp(times=0:20,t0=5)
    @test_throws "times must be nondecreasing" pomp(t0=0,times=[-t for t in 0:7])
    @test_throws "same elementary type" pomp(times=0:20,t0=0.0)

    P = pomp(times=0:20,t0=0);
    @test P isa POMP.PompObject
    @test timezero(P)==0
    @test times(P) isa Vector{<:Real}
    @test length(times(P))==21
    @test isnothing(obs(P))
    @test isnothing(states(P))

    p1 = (a=1.0,k=7.0,x0=5.0);
    p2 = (a=1.1,k=2.0,x0=3.0);

    x0 = rinit(P,params=p1,nsim=3);
    @test x0 isa Array{@NamedTuple{},2}
    @test size(x0)==(1,3)
    @test x0[1]==(;)
    rinit!(P,x0,params=p1);
    @test x0 isa Array{@NamedTuple{},2}
    @test size(x0)==(1,3)
    @test x0[1]==(;)

    P = pomp(P,rinit=rin);
    x0 = rinit(P,params=[p1;p2],nsim=7);
    @test x0 isa Array{<:NamedTuple,2}
    @test size(x0)==(2,7)
    @test keys(x0[8])==(:x,)
    @test_throws r"keyword argument .* not assigned" rinit(P,params=(a=1.0,k=7.0))
    @test_throws r"keyword argument .* not assigned" rinit!(P,x0,params=(a=1.0,k=7.0))

    x = rprocess(P,x0=x0,params=[p1;p2]);
    @test x isa Array{<:NamedTuple,3}
    @test size(x)==(21,2,7)
    @test x[21,:,:]==x0

    P = pomp(P,rprocess=discrete_time(rlin,dt=1));
    x = rprocess(P,x0=x0,params=[p1;p2]);
    @test x isa Array{<:NamedTuple,3}
    @test size(x)==(21,2,7)
    @test x[1,:,:]==x0
    @test x[21,:,:]!=x0
    @test keys(x[17])==(:x,)
    @test_throws r"keyword argument .* not assigned" rprocess(P,x0=x0[1,:],params=(k=7.0,x0=5.0))

    y = rmeasure(P,x=x,params=[p1;p2]);
    @test y isa Array{@NamedTuple{},3}
    @test size(y)==(21,2,7)
    @test y[3]==(;)

    P = pomp(P,rmeasure=rmeas);
    y = rmeasure(P,x=x,params=[p1;p2]);
    @test y isa Array{<:NamedTuple,3}
    @test size(y)==(21,2,7)
    @test keys(y[17])==(:y,)
    @test_throws r"keyword argument .* not assigned" rmeasure(P,x=x[:,:,1],params=(a=1.0,))

    ell = logdmeasure(P,x=x,y=y,params=[p1;p2]);
    @test ell isa Array{POMP.LogLik,4}
    @test size(ell)==(size(x)...,size(y,3))
    @test all(ell.==0.0)

    P = pomp(P,logdmeasure=logdmeas);
    ell = logdmeasure(P,x=x,y=y,params=[p1;p2]);
    @test ell isa Array{POMP.LogLik,4}
    @test size(ell)==(size(x)...,size(y,3))
    @test all(ell.<=0)
    @test_throws r"keyword argument .* not assigned" logdmeasure(P,y=y,x=x,params=(a=1.0,))

    ell = logdprior(P,params=[p1;p2]);
    @test ell isa Array{POMP.LogLik,1}
    @test size(ell)==(2,)
    @test all(ell.==0.0)

    P = pomp(P,logdprior=logdpri);
    ell = logdprior(P,params=[p1;p2]);
    @test ell isa Array{POMP.LogLik,1}
    @test size(ell)==(2,)
    @test all(ell.<=0)
    @test_throws r"keyword argument .* not assigned" logdprior(P,params=(a=1.0,))

    theta = rprior(P,params=[p1;p2],nsim=3);
    @test theta isa Matrix{<:NamedTuple}
    @test size(theta)==(2,3)
    @test theta[1,1]==p1
    @test theta[1,3]==p1
    @test theta[2,1]==p2

    P = pomp(P,rprior=rpri);
    theta = rprior(P,params=[p1;p2],nsim=4);
    @test theta isa Matrix{<:NamedTuple}
    @test size(theta)==(2,4)
    @test map(y -> y.x0, theta) isa Matrix{Float64}

    P = pomp(P,userdata=(other=3,))
    @test P isa POMP.PompObject

    ## merely for coverage purposes:
    ## do not try this at home!
    @test POMP.PompObject(P,init_state=x0[1],states=x[:,1,1]) isa POMP.PompObject
    @test POMP.PompObject(P) isa POMP.PompObject

end
